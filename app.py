...
def login_screen():
    ...
    if submitted:
        ...
        add_audit_log(user.get("login_id"), user.get("role"), "ログイン成功", "staff_accounts", user.get("id"), user.get("display_name"))
        st.rerun()
...
def password_change_screen(user: dict):
    ...
    if submitted:
        ...
        try:
            ...
            add_audit_log(user.get("login_id"), user.get("role"), "初回パスワード変更", "staff_accounts", user.get("id"))
...
def main():
    ...
    menus = ["ダッシュボード", "健康チェック", "排泄チェック", "業務全体申し送り"]
